...
template<typename T, int N, int M>
Matrix<T,N,M> operator+(const Matrix<T,N,M>& m1, const Matrix<T,0,0>& m2);
template<typename T, int N, int M>
Matrix<T,N,M> operator+(const Matrix<T,0,0>& m1, const Matrix<T,N,M>& m2);
template<typename T>
Matrix<T,0,0> operator+(const Matrix<T,0,0>& m1, const Matrix<T,0,0>& m2);

template<typename T, int N, int V, int M>
Matrix<T,0,0> operator*(const Matrix<T,N,V>& m1, const Matrix<T,V,M>& m2);
template<typename T, int N, int M>
Matrix<T,0,0> operator*(const Matrix<T,N,M>& m1, const Matrix<T,0,0>& m2);
template<typename T, int N, int M>
Matrix<T,0,0> operator*(const Matrix<T,0,0>& m1, const Matrix<T,N,M>& m2);
...
class Matrix{
    ...
    public:
        ...
        operator Matrix<T,0,0>();
};

template<typename T, int N, int M> 
Matrix<T,N,M>::operator Matrix<T,0,0>(){
    Matrix<T,0,0> newMatrix(M,N);
    for(int i = 1 ; i <= N ; i++){
        for(int j = 1 ; j <= M ;j++){
            newMatrix(i,j) = this->operator()(i,j);
        }
    }
    std::cout<<"conversion from static to dynamic\n";
    return newMatrix;
}
...
class Matrix<T,0,0>{
    ...
    public:
        ...
        operator Matrix<T,N,M>();
};

template<typename T>
template<int N, int M>
Matrix<T,0,0>::operator Matrix<T,N,M>(){
    if(this->numberOfRows() != N  || this->numberOfColumns() !=M){
        throw MatrixException("Incompatible sizes of matrices operation conversion dynamic to static\n");
    }
    Matrix<T,N,M> newMatrix;
    for(int i = 1 ; i <= N ; i++){
        for(int j = 1 ; j<= M ; j++){
    ...
    std::cout<<"conversion from dynamic to static\n";
    return newMatrix;
}
...
Matrix<T,N,M> operator+(const Matrix<T,N,M>& m1, const Matrix<T,0,0>& m2){
    if(N==m2.numberOfRows && M == m2.numberOfColumns){
        ...
        throw MatrixException("Incompatible sizes of matrices operation +");
    }
...
Matrix<T,0,0> operator+(const Matrix<T,0,0>& m1, const Matrix<T,0,0>& m2){
    ...
    else throw MatrixException("Incompatible sizes of matrices operation +");
}

template<typename T, int N, int V, int M>
Matrix<T,0,0> operator*(const Matrix<T,N,V>& m1, const Matrix<T,V,M>& m2){
    Matrix<T,0,0> newMatrix(N,M);
    for (int i = 1; i <= N; i++)
    {
        for (int j = 1; j <= M; j++)
        {
...
Matrix<T,0,0> operator*(const Matrix<T,N,M>& m1, const Matrix<T,0,0>& m2){
    ...
    for (int i = 1; i <= m1.numberOfRows(); i++)
    {
        ...
        {
            for (int  k = 1; i <= m2.numberOfRows(); k++)
            {
                newMatrix(i,j) += m1(i,k)*m2(k,j);
            }
...
template<typename T>
Matrix<T,0,0> operator*(const Matrix<T,0,0>& m1, const Matrix<T,0,0>& m2){
    Matrix<T,0,0> newMatrix(m1.numberOfRows(),m2.numberOfColumns());
    for (int i = 1; i <= m1.numberOfRows(); i++)
    {
        for (int j = 1; j <= m2.numberOfColumns(); j++)
        {
            for (int  k = 1; i <= m1.numberOfColumns(); k++)
            {
                newMatrix(i,j) += m1(i,k)*m2(k,j);
            }
